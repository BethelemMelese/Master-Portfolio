...
const ExperienceSection = () => {
  ...
  useEffect(() => {
    ...
    // Dot is positioned at top-8 (2rem) inside each card; align line to dot centers
    const dotOffset = 32; // px
    const dotRadius = 10; // approx radius of the dot

    const top = firstRect.top - containerRect.top + dotOffset + dotRadius / 2;
    const bottom = lastRect.top - containerRect.top + dotOffset + dotRadius / 2;
          ...
          {lineStyle && (
            <div className="pointer-events-none gap-4 md:gap-6 lg:gap-8 absolute left-1 -inset-y-3 flex justify-center">
              ...
              <motion.div
                ...
                className="absolute left-1 bottom-0 w-[4px] rounded-full origin-top"
                style={{
                  top: lineStyle.top,
                  height: lineStyle.height,
                  background:
                    "linear-gradient(to bottom, #f97373 0%, #ef4444 35%, #b91c1c 75%, rgba(185,28,28,0.2) 100%)",
                  boxShadow:
                    "0 0 8px rgba(248,113,113,0.7), 0 0 18px rgba(185,28,28,0.5)",
                }}
              />
            </div>
          )}
